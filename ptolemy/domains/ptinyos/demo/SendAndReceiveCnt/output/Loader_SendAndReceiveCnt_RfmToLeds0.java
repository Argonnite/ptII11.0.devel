...
public class Loader_SendAndReceiveCnt_RfmToLeds0 implements PtinyOSLoader {
    public void load(String path, PtinyOSDirector director) {
        String fileSeparator = System.getProperty("file.separator");
        String toBeLoaded = path + fileSeparator + System.mapLibraryName("_SendAndReceiveCnt_RfmToLeds0");
        toBeLoaded = toBeLoaded.replace('\\', '/');
        System.out.println("_SendAndReceiveCnt_RfmToLeds0.java : about to load " + toBeLoaded);
        System.load(toBeLoaded);
        this.director = director;
    }
    public int main(String argsToMain[]) throws InternalErrorException {
        return main_SendAndReceiveCnt_RfmToLeds0(argsToMain);
    }
    ...
    public void wrapup() {
        wrapup_SendAndReceiveCnt_RfmToLeds0();
    }
    public void processEvent(long currentTime) {
        processEvent_SendAndReceiveCnt_RfmToLeds0(currentTime);
    }
    public void receivePacket(long currentTime, String packet) {
        receivePacket_SendAndReceiveCnt_RfmToLeds0(currentTime, packet);
    }
    ...
    private native int main_SendAndReceiveCnt_RfmToLeds0(String argsToMain[]) throws InternalErrorException;
    private native void commandReadThread_SendAndReceiveCnt_RfmToLeds0();
    private native void eventAcceptThread_SendAndReceiveCnt_RfmToLeds0();
    private native void wrapup_SendAndReceiveCnt_RfmToLeds0();
    private native void processEvent_SendAndReceiveCnt_RfmToLeds0(long currentTime);
    private native void receivePacket_SendAndReceiveCnt_RfmToLeds0(long currentTime, String packet);
    ...
    class CommandReadThread extends Thread {
         public void run() {
             commandReadThread_SendAndReceiveCnt_RfmToLeds0();
         }
    }
    class EventAcceptThread extends Thread {
         public void run() {
             eventAcceptThread_SendAndReceiveCnt_RfmToLeds0();
         }
    }
}
