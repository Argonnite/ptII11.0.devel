...
public class Loader_RfmToLeds_MicaCompositeActor0 implements PtinyOSLoader {
    public void load(String path, PtinyOSDirector director) {
        String fileSeparator = System.getProperty("file.separator");
        String toBeLoaded = path + fileSeparator + System.mapLibraryName("_RfmToLeds_MicaCompositeActor0");
        toBeLoaded = toBeLoaded.replace('\\', '/');
        System.out.println("_RfmToLeds_MicaCompositeActor0.java : about to load " + toBeLoaded);
        System.load(toBeLoaded);
        this.director = director;
    }
    public int main(String argsToMain[]) throws InternalErrorException {
        return main_RfmToLeds_MicaCompositeActor0(argsToMain);
    }
    ...
    public void wrapup() {
        wrapup_RfmToLeds_MicaCompositeActor0();
    }
    public void processEvent(long currentTime) {
        processEvent_RfmToLeds_MicaCompositeActor0(currentTime);
    }
    public void receivePacket(long currentTime, String packet) {
        receivePacket_RfmToLeds_MicaCompositeActor0(currentTime, packet);
    }
    ...
    private native int main_RfmToLeds_MicaCompositeActor0(String argsToMain[]) throws InternalErrorException;
    private native void commandReadThread_RfmToLeds_MicaCompositeActor0();
    private native void eventAcceptThread_RfmToLeds_MicaCompositeActor0();
    private native void wrapup_RfmToLeds_MicaCompositeActor0();
    private native void processEvent_RfmToLeds_MicaCompositeActor0(long currentTime);
    private native void receivePacket_RfmToLeds_MicaCompositeActor0(long currentTime, String packet);
    ...
    class CommandReadThread extends Thread {
         public void run() {
             commandReadThread_RfmToLeds_MicaCompositeActor0();
         }
    }
    class EventAcceptThread extends Thread {
         public void run() {
             eventAcceptThread_RfmToLeds_MicaCompositeActor0();
         }
    }
}
